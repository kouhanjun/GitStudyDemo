...
iRPCClusterContainer iRPCClusterizer::doAction(const RPCDigiCollection::Range& digiRange, iRPCInfo& info)
{
	...
	/*khj: map的元素是pair，pair可以理解为结构体*/
    auto it = hits.begin();
	/*khj: auto 根据肤质表达式自定义数据类型*/
    ...
    for(auto digi = digiRange.first; digi != digiRange.second; ++digi) {
		...
		/*khj: 第一个second是第一个make_pair的第二个值，第二个second是第二个make_pair的第二个值*/
		/*khj: push_back是将元素放在容器的最后一位，back是返回容器最后一个元素的值*/
